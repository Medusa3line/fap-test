...
class Receipt extends Component {
	...
	componentDidMount = async () => {
		...
		if (!this.props.history.location.state){
			...
			await this.setState({DTO: this.props.history.location.state.agent.depositDTO})
			
			await this.setState({
				transaction: this.props.history.location.state.agent,
				tranDate: this.props.history.location.state.agent.tranDate
			})
	...
	render (){
		const { transactionType, statusdescription, amount, tranId, bankFrom, bankTo, beneficiary, itexTranId, tranDate } = this.state.transaction;
		...
		<div id="print-receipt">
	        <div className="form-horizontal" style={{textAlign: 'left'}} id="deposit-fields-2">
		        ...
		        <div className="form-group">
		            <div className="col-sm-12 col-md-12 col-lg-12">
		              <h5>Date<span> {tranDate !== undefined ? tranDate.substring(0,10) : null} </span></h5>
		            </div>
		        </div>
				<div className="form-group">
		            <div className="col-sm-12 col-md-12 col-lg-12">
		              <h5>Time <span> {tranDate !== undefined ? tranDate.substring(11,19) : null} </span></h5>
		            </div>
		        </div>
		        <div className="form-group">
		            <div className="col-sm-12 col-md-12 col-lg-12">
