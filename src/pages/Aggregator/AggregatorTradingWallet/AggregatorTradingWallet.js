...
class AggregatorTradingWallet extends Component {
  _isMounted = false;
  constructor(){
    super()
    this.state = {
  ...
  fetchHistory = async (reqBody) => {
    ...
    await fetch(`${baseUrl}/wallet/walletHistory/trading/${this.state.AgentId}`, {
      method: 'post',
      headers: {
...
export default withTimeout(withRouter(AggregatorTradingWallet));                       